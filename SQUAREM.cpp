...
// [[Rcpp::export]]
Rcpp::List SQUAREM(NumericVector par,NumericVector testingY,Function fixptfn,Function objfn)
{
    List sqobj;
    if (objfn!=NULL) {
        if (K == 1){
...
List squarem1(NumericVector par,Function fixptfn,Function objfn,NumericVector testingY){
    ...
    while(feval<maxiter){
        ...
        sr2=q1;
        sr2_scalar=0;
        for (int i=0;i<sr2.length();i++){sr2_scalar+=sr2[i]*sr2[i];}
        ...
        if(std::abs(alpha-1)>0.01){
            try{pnew=fixptfn(pnew,testingY);feval++;}
                ...
                if(trace){std::cout<<"Objective fn: "<<lnewcpp[0]<<"  Extrapolation: "<<extrap<<"  Steplength: "<<alpha<<"\n"<<std::endl;}
                iter++;
